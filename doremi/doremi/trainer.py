...
class DoReMiTrainer(Trainer):
    ...
    def read_weights(self):
        return self.model.train_domain_weights.clone().to('cuda')
    ...
    def update_domain_weights(self, scores, scores_mask, domain_ids):
        ...
        if self.args.doremi_optimizer == 'doremiv1':
            ...
            for domain_id in range(len(train_domain_weights)):
                domain_mask = (domain_ids == domain_id)[:, 0]
    ...
    def training_step(self, model, inputs):
        ...
        if self.args.reweight_domains:
            ...
            if self.args.doremi_optimizer == 'doremiv1':
                ...
                curr_domain_weights = train_domain_weights[inputs['domain_ids'][:, 0]].unsqueeze(-1).expand_as(pertoken_loss).detach()
        ...
        if self.do_grad_scaling:
            ...
            loss.backward()
        loss_csv = '/root/diplom_doremi/doremi/logs/'
        if self.args.reweight_domains:
            loss_csv += 'loss_doremi.csv'
        else:
            loss_csv += 'loss_baseline.csv'

        with open(loss_csv, 'a') as f_object:
            ...
            # the writerow()
            writer_object.writerow([loss.detach().item()])
    ...
    def evaluation_loop(
        ...
        for step, inputs in tqdm(enumerate(dataloader)):
            ...
            for domain_idx, domain_name in enumerate(self.eval_domain_list):
                domain_mask = (domain_ids == domain_idx)[:, 0]
