...
REFERENCE_WEIGHTS_NAME=${2:-"pile_doremi_data_200k"}
# name of the reference model to load (can be different from reference weights in later rounds)
REFERENCE_MODEL_NAME=${3:-"pile_baseline_data_200k_120M"}
OTHER_ARGS=$4
echo ${MODEL_OUTPUT_DIR}/${REFERENCE_MODEL_NAME}
...
accelerate launch \
    ...
    --do_train \
    --cache_dir ${CACHE} \
    --dataset_dir "/root/diplom_doremi/doremi/data_preprocessed" \
    --domain_config_path configs/pile_doremi_data_200k.json \
    ...
    --reference_model_name_or_path ${MODEL_OUTPUT_DIR}/${REFERENCE_MODEL_NAME}/checkpoint-15 \
    --bf16 \
    --shuffle \
